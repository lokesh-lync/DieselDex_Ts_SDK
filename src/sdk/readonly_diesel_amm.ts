...
export class ReadonlyDieselAmm {
  provider: Provider
  ammContract: DieselAmmContract

  constructor(provider: Provider, contractIdOpt?: string) {
    let contractId = contractIdOpt ?? DEFAULT_AMM_CONTRACT_ID
    this.provider = provider
    this.ammContract = new DieselAmmContract(contractId, provider)
  }
  ...
  async getAmountsOut(
    ...
    for (let poolId of pools) {
      ...
      let [assetOut, reserveIn, reserveOut, decimalsIn, decimalsOut] =
        arrangePoolParams(pool, assetIn)
  ...
  async getAmountsIn(
    ...
    for (let poolId of pools.reverse()) {
      ...
      let [assetIn, reserveOut, reserveIn, decimalsOut, decimalsIn] =
        arrangePoolParams(pool, assetOut)
