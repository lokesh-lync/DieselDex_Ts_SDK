...
import { DieselAmmContract } from './typegen/DieselAmmContract'
...
import { DieselAmmContractFactory } from './typegen/DieselAmmContractFactory'

export class DieselAmm {
  private readonly account: Account
  private readonly ammContract: DieselAmmContract
  ...
  constructor(account: Account, contractIdOpt?: string) {
    ...
    this.ammContract = new DieselAmmContract(contractId, account)
  ...
  static async deploy(wallet: Account): Promise<DieselAmm> {
    const { waitForResult } = await DieselAmmContractFactory.deploy(wallet)
    const { contract, transactionResult } = await waitForResult()
    console.log(
      'Deployed DieselAmm contract with status:',
      transactionResult.status,
      'and id:',
      contract.id.toB256()
    )
    return new DieselAmm(wallet, contract.id.toB256())
  }
