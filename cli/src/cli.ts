...
const FALLBACK_RPC = "https://mainnet.fuel.network/v1/graphql";
...
const diesel = new DieselAmm(wallet, CONTRACT_ADDRESS);
const readonlyDiesel = new ReadonlyDieselAmm(provider, CONTRACT_ADDRESS);
...
program
  ...
  .action(
    async (
      ...
      const request = await diesel.createPoolAndAddLiquidity(
        token0Contract,
        token0SubId,
        token1Contract,
        token1SubId,
...
program
  ...
  .action(
    async (
      ...
      const request = await diesel.createPool(
        token0Contract,
        token0SubId,
        token1Contract,
        token1SubId,
...
program
  ...
  .action(async (isStable, assetA, assetB, amountA, amountB) => {
    ...
    const deadline = await futureDeadline(provider);
    const request = await diesel.addLiquidity(
      poolId,
      amount0,
      amount1,
      0,
...
program
  ...
  .action(async (isStable, assetA, assetB, liquidity) => {
    ...
    const poolId = buildPoolId(assetA, assetB, isStable);
    const deadline = await futureDeadline(provider);
    const request = await diesel.removeLiquidity(
      poolId,
      liquidity,
      0,
      0,
...
program
  ...
  .action(async (isStable, assetA, assetB, maxInput, output) => {
    ...
    const poolId = buildPoolId(assetA, assetB, isStable);
    const deadline = await futureDeadline(provider);
    const request = await diesel.swapExactOutput(
      output,
      { bits: assetB },
      maxInput,
      [poolId],
...
program
  ...
  .action(async (isStable, assetA, assetB) => {
    isStable = isStable === "true";
    const poolId = buildPoolId(assetA, assetB, isStable);
    const meta = await readonlyDiesel.poolMetadata(poolId);
    if (meta === null) {
      console.log("Pool not found");
      return;
    }
    const lpAssetInfo = await readonlyDiesel.lpAssetInfo({
...
program.command("amm-meta").action(async () => {
  const meta = await readonlyDiesel.ammMetadata();
...
program.command("transfer-ownership newOwner").action(async (newOwner) => {
  let request = await diesel.transferOwnership(newOwner, txParams);
...
program.command("set-hook contractId").action(async (contractId?) => {
  ...
  let request = await diesel.setHook(contractId ?? undefined, txParams);
  await send(request, "set-hook");
});

program.command("deploy").action(async () => {
  await DieselAmm.deploy(wallet);
});
